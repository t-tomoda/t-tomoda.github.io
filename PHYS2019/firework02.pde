//　花火（サウンド付き）                                    T. Tomoda, 2019/6/18

float r = 5, x, y, delta_y = 3; //親玉の半径、位置、速度
int count_draw = 0;             //関数draw()が呼ばれる回数のカウント
int NUM = 1000;                 //小玉の数
Ball[] ball = new Ball[NUM];    //小玉の配列を生成

import ddf.minim.*; //サウンド処理用Minimライブラリーをロード
Minim minim;        //Minimオブジェクト
AudioPlayer player; //再生用Playerオブジェクト
...
void setup() {
  ...
  minim = new Minim(this);                 //サウンド用のMinimオブジェクト生成
  player = minim.loadFile("firework.mp3"); //サウンドファイルのロード
  player.play();                           //サウンド再生開始
}
...
void draw() {
  ...
  if (count_draw < 300) { //最初の5秒間（親玉の上昇）
    ...
    ellipse(x, y, r, r);          //(x, y)を中心とする半径rの円を描画
  } else {                //5秒後以降
